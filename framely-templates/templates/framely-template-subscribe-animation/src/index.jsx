...
export function SubscribeAnimation({
  ...
  const bellRotate = interpolate(frame - bellDelay, [0, 10, 20, 30, 40], [0, -20, 20, -10, 0], { extrapolateRight: 'clamp' });
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 20, transform: `scale(${buttonScale}) translateY(${buttonY}px)` }}>
        <button
          style={{
            padding: '16px 32px', fontSize: 24, fontWeight: 600, color: '#fff',
            background: showSubscribed ? '#666' : buttonColor,
            border: 'none', borderRadius: 4, cursor: 'pointer', minWidth: 200,
          }}
        >
          {showSubscribed ? 'SUBSCRIBED' : 'SUBSCRIBE'}
        </button>
        {showBell && (
          <div style={{ width: 50, height: 50, background: 'rgba(255,255,255,0.1)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', transform: `scale(${bellScale}) rotate(${bellRotate}deg)` }}>
      ...
      <div style={{ opacity: nameOpacity, transform: `translateY(${nameY}px)`, textAlign: 'center' }}>
        <p style={{ fontSize: 18, color: 'rgba(255,255,255,0.6)', margin: 0 }}>Thanks for subscribing to</p>
        <p style={{ fontSize: 28, fontWeight: 600, color: '#fff', margin: '8px 0 0' }}>{channelName}</p>
      </div>
      <div style={{ position: 'absolute', left: '50%', top: '50%', transform: `translate(${cursorX}px, ${cursorY}px)`, opacity: cursorOpacity, pointerEvents: 'none' }}>
        <svg width="32" height="32" viewBox="0 0 24 24" style={{ transform: cursorClick ? 'scale(0.9)' : 'scale(1)' }}>
          <path d="M4 4l16 12-6.5 1.5L12 24 4 4z" fill="#fff" stroke="#000" strokeWidth="1" />
        </svg>
      </div>
      {showSubscribed && [...Array(12)].map((_, i) => {
        ...
        return (
          <div key={i} style={{ position: 'absolute', left: '50%', top: '45%', width: 8, height: 8, background: buttonColor, borderRadius: '50%', transform: `translate(${Math.cos(angle) * distance}px, ${Math.sin(angle) * distance}px)`, opacity: particleOpacity }} />
        );
      })}
    </AbsoluteFill>
  );
