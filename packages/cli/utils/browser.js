...
export async function setFrame(page, frame, timeout = DEFAULT_TIMEOUT) {
  // Set frame and check delayRender in a single IPC roundtrip.
  // __setFrame uses flushSync, so the DOM is committed before we check delayRender.
  ...
  // Only poll if there are actually pending async resources
  if (hasPendingDelays) {
    try {
      await page.waitForFunction(
        () => {
